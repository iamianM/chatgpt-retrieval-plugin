...
def depth_score_to_topic_change_indexes(
    ...
    print(len(local_maxima_indices), len(local_maxima))
...
def topic_segmentation_with_existing_embeddings(
    paragraph_embeddings,
    textiling_hyperparameters: TextTilingHyperparameters,
    topicsegmentation_config: TopicSegmentationConfig,
):
    # paragraph_embeddings = [torch.tensor([e]) for e in paragraph_embeddings]
    # print(paragraph_embeddings[0].shape)
    # Compute block comparison scores based on the given paragraph embeddings
    block_comparison_score_timeseries = block_comparison_score(
        paragraph_embeddings, k=textiling_hyperparameters.SENTENCE_COMPARISON_WINDOW
    )
    print(len(block_comparison_score_timeseries))

    # # Smooth the block comparison score time series
    # block_comparison_score_timeseries = smooth(
    #     block_comparison_score_timeseries,
    #     n=textiling_hyperparameters.SMOOTHING_PASSES,
    #     s=textiling_hyperparameters.SMOOTHING_WINDOW,
    ...
    depth_score_timeseries = depth_score(block_comparison_score_timeseries)
    print(len(depth_score_timeseries))

    # Get the topic change indices based on the depth scores
    topic_change_indices = depth_score_to_topic_change_indexes(
        depth_score_timeseries,
        len(paragraph_embeddings),  # You can replace this with an appropriate duration if needed
        topic_segmentation_configs=topicsegmentation_config,  # Adjust this configuration if needed
    )
